...
def get_matrix(my_data): #多个对象的相容类等于单个对象的交集181
    a = time.perf_counter()
...
def Dynamic_matrix_construct(pre_DM,pre_pos_list,new_pos_list,pre_dec_list,new_dec_list,con_data,dec_data,sp_list):
...
def Matrix_construct(con_data,pos_list,dec_data):  #构造基于正域的矩阵
    ...
    return DM
def Matrix_construct_2(con_data,pos_list,dec_data,sp_list):  #构造基于正域的矩阵
...
if __name__ == '__main__':
    ...
    print(type(eval(con_data[0][0])),eval(con_data[0][0]))
    ...
    start = time.perf_counter()
    #####找最小
    sp_list = get_matrix(con_data)
    st = time.perf_counter()
    print("第二次，矩阵出来了", st - start)
    con_divlist = div_base_matric(sp_list)
    print("第二次", time.perf_counter() - st)
...
'''
    ...
    start_2 = time.perf_counter()
    new_DM = Dynamic_matrix_construct(DM_1, pos_list_1, pos_list_2, dec_divlist_1[class_num_1], dec_divlist_2[class_num_2],con_data,dec_data_2,sp_list)
    ...
    DM_3 = Matrix_construct_2(con_data, pos_list_2, dec_data_2, sp_list)
    end_6 =  time.perf_counter()
    reduct_list_2 = Red(DM_3)
    print("老方法构造时间:", end_6 - start_6,"老方法析取合取时间:", time.perf_counter() - end_6)
    # reduct_list_2 = Red(DM_2)
