...
if __name__ == '__main__':
    start = time.perf_counter()
    list_data = readfileBylist("../set_value_dataSet(5%)在修改/set_value.txt")
    ...
    # 构造矩阵时间 5.503694222
    print("k=8")
    pos_list_8 = pos(dec_divlist_2, con_divlist)
    DM_8 = Matrix_construct(con_data, pos_list_8, dec_data_2)
    reduct_list = Red(DM_8)
    red_avgLength(reduct_list)

    print("k=16")
    pos_list_16 = pos(dec_divlist_3, con_divlist)
    DM_16 = Matrix_construct(con_data, pos_list_16, dec_data_3)
    reduct_list = Red(DM_16)
    red_avgLength(reduct_list)
    '''
    ##    单类K=4
