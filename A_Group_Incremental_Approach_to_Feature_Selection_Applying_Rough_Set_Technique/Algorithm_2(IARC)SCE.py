...
def U_Ux_SCE_Entropy(U_con_data,U_dec_data,Ux_con_data,Ux_dec_data):
    ...
    U_Ux_con_divlist, U_Ux_dec_divlist = merge_divlist(U_con_data, Ux_con_data, U_dec_data, Ux_dec_data, U_con_divlist, U_dec_divlist,
                             Ux_con_divlist, Ux_dec_divlist)
    intersect_len =len(set(U_Ux_con_divlist) & set(U_Ux_dec_divlist)) 
    latter = 0
    if len(U_Ux_con_divlist) - 1 != 0:
        for j in U_dec_divlist:
            latter += len((set(U_Ux_con_divlist) - set(sum(Ux_con_divlist,[]))) & set(j)) * math.log10(((len(U_Ux_con_divlist) - 1) / len(U_Ux_con_divlist)))
        latter += (intersect_len - 1) * math.log10(((len(U_Ux_con_divlist) - 1) * intersect_len)/len(U_Ux_con_divlist) * (intersect_len - 1)) + math.log10(intersect_len / len(U_Ux_con_divlist))
    print((U_len *  SCE_Entropy(U_con_divlist,U_dec_divlist) - latter)/(U_len + 1),latter)
    return (U_len * SCE_Entropy(U_con_divlist,U_dec_divlist) - latter)/(U_len + 1)
...
def merge_divlist(U_con_data,Ux_con_data,U_dec_data,Ux_dec_data,U_con_divlist,U_dec_divlist,Ux_con_divlist,Ux_dec_divlist):#      U/C + Ux/C
    ...
    return U_Ux_con_divlist,U_Ux_dec_divlist
