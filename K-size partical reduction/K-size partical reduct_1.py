...
def GenerateSolution(con_data,dec_data,red_data,red_list,pos_list,weight_list):
        ...
        for i in range(len(attr_list)):
            ...
            for i in range(len(cmp_list)):
                ...
                if cmp_list[x] != cmp_list[i] and temp_dec_data[x] != temp_dec_data[i]:
                    num += 1
            dic_list += [num]
        max_num = max(dic_list)
        for i in range(len(dic_list)):
            if max_num == dic_list[i]:
                add_filter += [attr_list[i]]
    #删除集
    del_filter = []
    random_add = random.randint(0, len(add_filter) - 1)
    red_data = data_add(attr_data, red_data, attr_list.index(add_filter[random_add]))
    ...
    if len(del_filter) == 0:
        ...
        for i in range(len(red_list)):
            ...
            sig_list += [sig_inter]
        max_num = max(sig_list)
        for i in range(len(sig_list)):
            if max_num == sig_list[i]:
                del_filter += [red_list[i]]
    random_del = random.randint(0, len(del_filter) - 1)
    red_list += [add_filter[random_add]]
    red_data = deal_data(red_data,red_list.index(del_filter[random_del]))
    del red_list[red_list.index(del_filter[random_del])]
